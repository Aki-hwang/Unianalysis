...
- 보유 장비/시술: 울쎄라, 써마지, 올리지오, 실리프팅, 보톡스, 필러, 스킨부스터(리쥬란), 레이저 제모(아포지 플러스), 색소 치료
- 위치: 김포시 김포한강4로 525 감두리빌딩 2층 (구래동, 구래역 인근) · 031-8049-6024

## 자동 측정 루틴

- 3일에 1회, 새벽 3시(KST) 웹 검색 기반으로 index.html의 PROMPTS 전체를 측정 (cron: 0 18 */3 * * UTC). 노출 개선 액션 시작 시 2일 1회로 상향 예정
